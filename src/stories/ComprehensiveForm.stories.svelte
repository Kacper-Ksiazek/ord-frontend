				...
				>
						...
						<div class="space-y-6">
							<div class="flex items-center gap-3 pb-4 border-b dark:border-gray-700">
								...
								<div>
									...
									<p class="text-gray-600 dark:text-gray-400">Tell us about yourself</p>
								</div>
							</div>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									...
									{#if formErrors.firstName}
										<Helper class="mt-2" color="red">{formErrors.firstName}</Helper>
									{/if}
								</div>
								...
								<div>
									...
									{#if formErrors.lastName}
										<Helper class="mt-2" color="red">{formErrors.lastName}</Helper>
									{/if}
								</div>
							</div>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div>
									...
									<div class="flex gap-4">
										<Radio name="gender" bind:group={formData.gender} value="male">Male</Radio>
										<Radio name="gender" bind:group={formData.gender} value="female">Female</Radio>
										<Radio name="gender" bind:group={formData.gender} value="other">Other</Radio>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="space-y-6">
							<div class="flex items-center gap-3 pb-4 border-b dark:border-gray-700">
								...
								<div>
									...
									<p class="text-gray-600 dark:text-gray-400">Where are you located?</p>
								</div>
							</div>

							<div>
								<Label for="street" class="mb-2">Street Address</Label>
								<Input id="street" bind:value={formData.street} placeholder="123 Main Street, Apt 4B" />
							</div>

							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									<Label for="city" class="mb-2">City</Label>
									<Input id="city" bind:value={formData.city} placeholder="San Francisco" />
								</div>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									...
									<Input id="zipCode" bind:value={formData.zipCode} placeholder="94102" />
								</div>
						...
						<div class="space-y-6">
							<div class="flex items-center gap-3 pb-4 border-b dark:border-gray-700">
								...
								<div>
									...
									<p class="text-gray-600 dark:text-gray-400">Your work details</p>
								</div>
							</div>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									...
									<Input id="company" bind:value={formData.company} placeholder="Acme Corp" />
								</div>

								<div>
									<Label for="position" class="mb-2">Job Position</Label>
									<Input id="position" bind:value={formData.position} placeholder="Senior Developer" />
								</div>
							</div>
						...
						<div class="space-y-6">
							<div class="flex items-center gap-3 pb-4 border-b dark:border-gray-700">
								...
								<div>
									...
									<p class="text-gray-600 dark:text-gray-400">Create your login credentials</p>
								</div>
							</div>
							...
							<div>
								...
								{#if formErrors.confirmPassword}
									<Helper class="mt-2" color="red">{formErrors.confirmPassword}</Helper>
								{/if}
							</div>
						</div>
					{/if}
					...
					{#if currentStep === 5}
						<div class="space-y-6">
							...
							<div class="space-y-4">
								<Toggle bind:checked={formData.newsletter}>Subscribe to newsletter</Toggle>
								<Toggle bind:checked={formData.notifications}>Enable email notifications</Toggle>
								<Toggle bind:checked={formData.twoFactor}>Enable two-factor authentication</Toggle>
							</div>

							<div>
								<Label class="mb-3">Preferred Contact Method</Label>
								<div class="space-y-2">
									<Radio name="contact" bind:group={formData.preferredContact} value="email">Email</Radio>
									<Radio name="contact" bind:group={formData.preferredContact} value="phone">Phone</Radio>
									<Radio name="contact" bind:group={formData.preferredContact} value="sms">SMS</Radio>
									<Radio name="contact" bind:group={formData.preferredContact} value="none">No contact</Radio
									>
								</div>
							</div>
								...
								<div class="grid grid-cols-2 md:grid-cols-4 gap-2">
									{#each ['JavaScript', 'TypeScript', 'React', 'Vue', 'Svelte', 'Node.js', 'Python', 'Java'] as skill}
										<Button
											...
											color={formData.skills.includes(skill) ? 'blue' : 'alternative'}
							...
							<div class="space-y-3 pt-4 border-t dark:border-gray-700">
								...
								>
									I agree to the <button
										type="button"
										onclick={(e) => e.preventDefault()}
										class="text-blue-600 hover:underline">Terms of Service</button
									>
									<span class="text-red-500">*</span>
								</Checkbox>
						...
						<div class="space-y-6">
							<div class="flex items-center gap-3 pb-4 border-b dark:border-gray-700">
								...
								<div>
									...
									<p class="text-gray-600 dark:text-gray-400">Secure payment details</p>
								</div>
							</div>
							...
							<div>
								...
								<Input id="cardName" bind:value={formData.cardName} placeholder="JOHN DOE" />
							</div>
					...
					<div class="flex justify-between items-center pt-6 border-t dark:border-gray-700">
						<div>
							{#if currentStep > 1}
								<Button type="button" color="alternative" onclick={prevStep}>Previous</Button>
							{/if}
						</div>
