<script lang="ts">
	...
	const selectAllLabel = $derived(
		allPageSelected
			? m['features.words.inbox.bulk.unselect_all']()
			: m['features.words.inbox.bulk.select_all']()
	);
...
<div
	...
	role="toolbar"
	aria-label={m['features.words.inbox.bulk.toolbar_aria_label']()}
>
	<Button
		type="FILLED"
		variant="TEXT"
	...
	>
		...
		<span>{m['features.words.inbox.bulk.activate_selected']()}</span>
	</Button>
	...
	>
		...
		<span>{m['features.words.inbox.bulk.remove_selected']()}</span>
	</Button>
</div>
