...
<div class="mx-auto w-full max-w-sm px-4" data-testid={E2E_TEST_IDS.login.page}>
	<p class="mb-3 text-center text-[11px] font-medium tracking-[0.2em] text-ink-subtle">ORD</p>

	<h1 class="mb-2 text-center text-[32px] font-medium leading-10 tracking-tight text-ink">
		{step === 'email' ? m['auth.login.title']() : m['auth.login.verify_otp_title']()}
	</h1>
	<p class="mb-8 text-center text-[15px] leading-[22px] text-ink-muted">
